...
-(void)image{
    ...
    NSData * dataImage;
  keyImgView.image=[ud dataForKey:@"imageKey"];
...
-(void)up{
    ...
    if(min== 0&& sec==0){
        //次のviewdid loadに移動
        [timer invalidate];
        [self performSegueWithIdentifier:@"trans" sender:nil];
    }
...
- (void)viewDidLoad {
    ...
    NSData * dataImage;
    size = 50;
    dataImage = [ud dataForKey:@"imageKey"];
    
    size = 50; // パズルピースの画像の大きさ
    UIImage *image_mae = [[UIImage alloc] initWithData:dataImage]; // サンプル画像を読み込む
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ( [[segue identifier] isEqualToString:@"trans"] ) {
        ...
        nextViewController.receiveString = isClear;
    }
}
